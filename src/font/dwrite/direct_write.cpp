...
// Arena-backed growable arrays. The header sits right before the elements.
// Note that 'len' is the capacity and 'cap' is the count.
struct Array_Header {
    u64 len;
    u64 cap;
};
...
static void _arrsetlen(Arena *arena, void **arr, u64 size, u64 count)
{
    if (*arr == 0)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            memcpy(new_base, *arr, size*header->cap);
...
static void fp_atlas_upload(Fp_Atlas *atlas)
{
    gfx_texture_upload(atlas->id, RHI_FORMAT_RGBA8_UNORM, atlas->data, atlas->pitch * atlas->height, atlas->width, atlas->height);
}
...
Fp_Run *dwrite_runs_from_string(String string, String base_family8, f32 font_size)
{
    ...
    while (offset < string16.len)
    {
        ...
        if (font == NULL)
        {
            ...
            font->atlas.id = guid_generate();
            {
                RHI_Texture_Desc desc = {};
                desc.name       = S("FontAtlas");
                desc.type       = RHI_TEXTURE_TYPE_2D;
                desc.format     = RHI_FORMAT_RGBA8_UNORM;
                desc.usage      = RHI_TEXTURE_USAGE_SAMPLED;
                desc.width      = font->atlas.width;
                desc.height     = font->atlas.height;
                desc.mip_levels = 1;
                desc.depth      = 1;
                gfx_texture_create(font->atlas.id, desc);
            }
            fp_atlas_upload(&font->atlas);
        }
            ...
            else // complex
            {
                     ...
                     analysis_sink_result = analysis_sink_result->next)
                {
                    ...
                    if (arrlenu(advances) < actual_glyph_count_next)
                    {
                        ...
                        size = max(size, (u64)actual_glyph_count_add);
...
void fp_pack_run(Fp_Run *run, b32 is_cleartype)
{
    ...
    for (u32 i = 0; i < glyph_count; ++i)
    {
        ...
        if (! entry) // glyph index doesn't exist in the inner-table
        {
                ...
                if (rpk_result.did_fit)
                {
                    ...
                    for (u32 r = 0; r < blackbox_height; ++r)
                    {
                        for (u32 c = 0; c < blackbox_width; ++c)
                        {
                            ...
                            {
                                // Grayscale coverage: white, coverage in alpha. Blends over anything.
                                u8 coverage = bitmap_data_rgb[r*blackbox_width + c];
                                dst[0] = 0xff;
                                dst[1] = 0xff;
                                dst[2] = 0xff;
                                dst[3] = coverage;
            ...
            {
                glyph->index  = glyph_index;
                glyph->uv_min = vec2(0.f);
                glyph->uv_max = vec2(0.f);
                glyph->lsb    = 0.f;
                glyph->rsb    = 0.f;
                glyph->tsb    = 0.f;
                glyph->bsb    = 0.f;
    ...
    if (font_entry->atlas.dirty)
    {
        ...
        fp_atlas_upload(&font_entry->atlas);
    }

    scratch_end(scratch);
}

Fp_Draw_String_Result fp_draw_string(String string, String base_family, f32 font_size, vec2 origin, Render_String_Flags flags, AABB2 cull_aabb)
{
    ProfileScope;
    ...
    AABB2 aabb      = {vec2( F32_MAX,  F32_MAX), vec2(-F32_MAX, -F32_MAX)};
    ...
    for (Fp_Run *run = runs; run != NULL; run = run->next)
    {
        ...
        for (u32 i = 0; i < run->count; ++i)
        {
            ...
            vec2 min = pen + vec2(glyph->lsb, glyph->tsb);
            vec2 max = pen + vec2(glyph->rsb, glyph->bsb);
            ...
            if (! (flags & RENDER_STRING_FLAG_NO_DRAW))
            {
                ...
                if (flags & RENDER_STRING_FLAG_CULL)
                {
                    ...
                    if (intersects(cull_aabb, cel))
                    {
                        ...
                        vec2 box_range_x = vec2(min.x, max.x);
                        vec2 box_range_y = vec2(min.y, max.y);
                        ...
                        draw_quad(overlap.min, overlap.max, uv_min, uv_max, vec4(1.f), atlas);
                    }
                }
                else
                {
                    draw_quad(min, max, uv_min, uv_max, vec4(1.f), atlas);
                }
            }
