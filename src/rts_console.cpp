...
internal void
console_init(Console *console, Asset_Font *font) //@Todo: assert font...
{
    if (! console->initted) 
    {
...
internal b32
console_cmd_equal(Console *console, char *str) 
{
    return string_equal(console->cbuf, console->cbuf_at, str, string_length(str));
}

internal void
console_draw(Console *console, Render_Group *group, f32 real_time, f32 width, f32 height) 
{
    if (console->dt != 0.0f) 
    {
        ...
        f32 blink_freq = pi32 / ((f32)os.caret_blink_time() / 1000.0f);
        v4 cursor_color = lerp(cursor->color1, 0.5f*sin(real_time*blink_freq) + 1.0f, cursor->color2);
...
internal void
console_update(Console *console, Game_State *game_state, Input *input) 
{
    if (console->is_down) 
    {
        ...
        if (toggled_down(input, KEY_ENTER)) {
#if 0
            if (console_cmd_equal(console, "freecam")) 
            {
            ...
            if (0) {}
#endif
            else if (console_cmd_equal(console, "exit")) 
            {
