                 ...
                 ++Y) 
            {
                     ...
                     ++X) 
                {
                    Chunk *chunk = get_chunk(&game_state->world_arena,
                                             &game_state->world->chunk_hashmap, {X, Y, Z});
...
load_bmp(Memory_Arena *arena, Read_Entire_File *read_file, const char *filename)
{
    ...
    *result = {};
    ...
    opengl_gen_linear_buffer(gl, &gl->octree_nodes, &gl->octree_nodes_texture, GL_R32UI, sizeof(u32) * max_cap);
...
end_work_memory(Work_Memory_Arena *workMemory_Arena)
{
    ...
    workMemory_Arena->is_used = false;
        ...
        for (u32 idx = 0; idx < array_count(transient_state->work_arenas); ++idx) {
...
#if 0
// @TODO: janky asf
if (hovered_entity_id) game_state->hot_entity_id = hovered_entity_id;
if (clicked_entity_id && !ui->context.active) game_state->active_entity_id = clicked_entity_id;
char entity_str[64];
snprintf(entity_str, array_count(entity_str), "Hot Entity ID: %u\nActive Entity ID: %u", game_state->hot_entity_id, game_state->active_entity_id);
string_op(STRING_OP_DRAW, orthographic_group, V3(V2(screen.resolution)*0.8f, 0), entity_str, &assets->debug_font);

if (game_state->active_entity_id) {
    Entity *entity = world->entities[game_state->active_entity_id - 1];
    if (entity) {
        v3 clip_pos = project(game_state->using_camera->VP, entity->world_translation);
        clip_pos.xy = (clip_pos.xy + V2(1, 1)) * 0.5f;
        clip_pos.xy = hadamard(clip_pos.xy, V2(draw_width, draw_height));
        clip_pos.z  = 0;

        v3 offset = ui->gizmo(3, clip_pos, entity->world_rotation);
        entity->world_translation += offset;

        snprintf(entity_str, array_count(entity_str), "ID:%u\nWorld Position: %.2f, %.2f, %.2f", entity->id, entity->world_translation.x, entity->world_translation.y, entity->world_translation.z);
        v3 p = clip_pos + V3(10,0,0);
        string_op(STRING_OP_DRAW, orthographic_group, p, entity_str, &assets->debug_font);
        string_op(STRING_OP_DRAW, orthographic_group, p + V3(3, -3, -1), entity_str, &assets->debug_font,RGBA_BLACK);
    }
}
#endif
...
#if 0
v3 gizmo(u32 id, v3 pos, qt rotation)
{
    v3 result = {};

    v2 half_dim = v2{8, 8};
    Rect2 rect = rect2_cen_half_dim(pos.xy, half_dim);
    v4 color = V4(0.7f, 0.7f, 0.2f, 1.0f);
    f32 c = 0.0002f;

    if (context.active == id) {
        if (!mouse->is_down[Mouse_Left] && mouse->toggle[Mouse_Left]) {
            context.active = 0;
            result.x = (input->mouse.pos.x - prev_mouse_pos.x) * c;
            result.y = (input->mouse.pos.y - prev_mouse_pos.y) * c;
        }
        else {
            result.x = (input->mouse.pos.x - prev_mouse_pos.x) * c;
            result.y = (input->mouse.pos.y - prev_mouse_pos.y) * c;
        ...
        if (mouse->is_down[Mouse_Left] && mouse->toggle[Mouse_Left] && in_rect(rect, mouse->pos)) {
            context.active = id;
            color.rgb *= 0.8f;
            prev_mouse_pos = input->mouse.pos;
        }
        else if (!in_rect(rect, mouse->pos)) {
            context.hot = 0;
        }
        else {
        }
        ...
        if (in_rect(rect, mouse->pos)) {
            context.hot = id;
            color.rgb *= 1.2f;
        }
    }

    push_outlined_rect(render_group, rect, 0.0f, color, 3.0f);
